...
```{r fig.width = 10, fig.fullwidth = TRUE}
staff_long %>%
  ggplot(aes(x = year, y = value, group = faculty_type, colour = faculty_type)) +
  geom_line() +
  labs(title = "Trends in faculty hiring in the USA from 1975 to 2011", subtitle = "according to data from the American Association of University Professors", x = "", y = "", colour = "Faculty type")
```
...
- *To highlight the fact that, unlike other instructional staff types, the proportion of part-time faculty members has increased, I would probably colour all the lines the same colour, except for the line for part-time faculty members, which would be a brighter colour, so that it would stand out against the others, and change the subtitle to note the new emphasis of the plot.*

3.  Implement the changes you proposed in the previous exercise.


```{r fig.width = 10, fig.fullwidth = TRUE}
#| label: faculty-graph-remake
staff_long %>%
  ggplot(aes(x = year, y = value, group = faculty_type, colour = faculty_type)) +
  geom_line() +
  labs(title = "Trends in faculty hiring in the USA from 1975 to 2011", subtitle = "Full-time vs. part-time faculty", x = "", y = "", colour = "Faculty type") +
  scale_colour_manual(values = c(rep("gray33", 4), "red")) +
  theme_minimal()
```

- *note: I also changed the theme of the graph to make the data more easily visible, so it wouldn't be grey lines on a grey background.*
...
- *For one, it would be better to not write out the graph as a line plot (or a fill plot), since this implies some sort of order and connection between the different values along the x-axis, while the list of countries doesn't actually have any intrinsic order.*
- *Also, pie charts are really hard to read, especially with so many smaller entries in them, perhaps replace these with bar charts?*
- *The threshold for inclusion in the visualisation should be raised, since most of the data in the graphs are completely unreadable due to how small they are in comparison to the rest. If, instead, you want to show more countries, it would probably be better to remove the major outliers (especially China).*
- *If you're going to use charts with some sort of an order, you should sort each chart by either a factor that is important in all charts or by a different factor if they don't have the same order.*
- *Some informative labels on the graph would be useful.*
- *Have some sort of a part of the data that is being focused on, not just the general, not-especially focused visualisations here.*
...
fisheries %>%
  ...
  labs(title = "Distribution of countries", subtitle = "by total amount of caught fish", x = "", y = "")

fisheries %>%
  filter(total >= 1000000, total <= 10000000) %>%
  mutate(country = fct_reorder(country, total)) %>%
  select(-total) %>%
  pivot_longer(-country, names_to = "type") %>%
  ggplot(aes(y = country, x = value, fill = type)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ type) +
  theme(legend.position = "none") +
  scale_fill_manual(values = c("#71b6c4","#9ecae1")) +
  labs(title = "Aquaculture vs. capture fish supply", subtitle = "in countries with between 1 and 10 million tons of caught fish", x = "", y = "")
